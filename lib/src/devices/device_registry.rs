...
use super::{device_id::DeviceId, device_session::DeviceSession};
...
impl DeviceRegistry {
    ...
    pub fn add(&mut self, id: DeviceId, session: DeviceSession) {
        self.devices.insert(id, session);
    }

    pub fn remove(&mut self, id: &DeviceId) -> Option<DeviceSession> {
        self.devices.remove(id)
    }

    pub fn get(&self, id: &DeviceId) -> Option<&DeviceSession> {
        self.devices.get(id)
    }

    pub fn get_mut(&mut self, id: &DeviceId) -> Option<&mut DeviceSession> {
    ...
    pub fn list(&self) -> impl Iterator<Item = &DeviceSession> {
        self.devices.values()
    }
...
mod tests {
    ...
    fn add_new_registry_device() {
        ...
        registry.add(device_id.clone(), session);
    ...
    fn get_mut_registry_device() {
        ...
        let device = registry.get_mut(&device_id).expect("device should exist");

        device.adapter.connect().expect("adapter should connect");
    ...
    fn gets_first_device_mut() {
        ...
        registry.add(device_id, session);

        let device = registry.first_mut().expect("device should exist");
