...
impl UsbAdapter for RusbAdapter {
    ...
    fn connect(&self, info: &UsbDeviceInfo) -> Result<Box<dyn UsbConnection>, UsbError> {
        ...
        let device = devices
            .iter()
            .find(|device| {
                device.bus_number() == info.bus_number && device.address() == info.address
            })
            .ok_or(UsbError::DeviceNotFound)?;

        Ok(Box::new(RusbConnection::new(device)))
    }
}
...
impl UsbConnection for RusbConnection {
    fn open(&mut self) -> Result<(), UsbError> {
        ...
        let config = self.device.active_config_descriptor().map_err(|error| {
    ...
    fn write(&mut self, endpoint: u8, data: &[u8]) -> Result<usize, UsbError> {
        let handle = self.handle.as_mut().ok_or(UsbError::NotOpen)?;

        match handle.write_bulk(endpoint, data, std::time::Duration::from_secs(1)) {
            Ok(size) => {
    ...
    fn read(&mut self, endpoint: u8, data: &mut [u8]) -> Result<usize, UsbError> {
        ...
        handle
            .read_bulk(endpoint, data, std::time::Duration::from_secs(1))
...
mod tests {
    ...
    fn discovers_connected_usb_devices() {
        ...
        for device in devices {
            println!(
                ...
                device.vendor_id, device.product_id, device.bus_number, device.address,
            );
    ...
    fn connection_fails_when_not_open() {
        ...
        let mut connection = adapter.connect(info).expect("Failed to create connection");

        let data = [1, 2, 3, 4];

        let result = connection.write(0x01, &data);

        assert!(matches!(result.unwrap_err(), UsbError::NotOpen));
    }
    ...
    fn close_closes_connection() {
        ...
        connection.open().expect("Failed to open connection");
